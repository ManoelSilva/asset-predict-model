...
class B3DashboardPlotter:
    ...
    def plot_action_samples_by_ticker(
        ...
        for i, ticker in enumerate(sample_tickers):
            subdf = df[df[ticker_col] == ticker].sort_values('date')
            prices = subdf[price_col]
            actions = subdf[target_col].astype(str).str.lower()
            axes[i].plot(subdf['date'], prices, label='Price', color='blue')
            ...
            axes[i].scatter(subdf.loc[buy_idx, 'date'], prices.loc[buy_idx], color='green', label='BUY', marker='^')
            axes[i].scatter(subdf.loc[sell_idx, 'date'], prices.loc[sell_idx], color='red', label='SELL', marker='v')
            axes[i].scatter(subdf.loc[hold_idx, 'date'], prices.loc[hold_idx], color='gray', label='HOLD', marker='o',
                            alpha=0.5)
