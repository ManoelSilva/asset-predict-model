...
class LSTMConfig(ModelConfig):
    ...
    def __init__(self, lookback: int = 32, horizon: int = 1, units: int = 64,
                 dropout: float = 0.4, learning_rate: float = 1e-3, epochs: int = 25,
                 batch_size: int = 128, loss_weight_action: float = 20.0,
                 loss_weight_return: float = 50.0, price_col: str = "close",
                 early_stopping_patience: int = 20, early_stopping_min_delta: float = 100.0,
                 gradient_clip_norm: float = 1.0, weight_decay: float = 1e-4,
                 lr_scheduler_patience: int = 5, lr_scheduler_factor: float = 0.5,
                 lr_scheduler_min_lr: float = 1e-6, use_class_weights: bool = False,
                 focal_loss_gamma: float = 3.0, **kwargs):
